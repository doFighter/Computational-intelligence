...
%% --------------------------------------------------------------------%%
function [minimal_path,minimal_length] = Symmetry_ACS(x,y,iterate_max)
    % 获取城市数目
    city_num = length(x);
    % 设置蚂蚁数目
    ants = 20;
    ...
    while iterate < iterate_max
        ...
        for i = 2:city_num
            for j = 1:ants
                ...
                tau(visit_city,start_city) = tau(start_city,visit_city);
        ...
        for i = 1:minimal_ants
            for j = 2:city_num
                ...
                tau(minimal_path(i,:)==j,minimal_path(i,:)==j-1) = tau(minimal_path(i,:)==j-1,minimal_path(i,:)==j);
            end
            tau(minimal_path(i,:)==city_num,minimal_path(i,:)==1) = tau(minimal_path(i,:)==city_num,minimal_path(i,:)==1) + alpha / minimal_length;
            tau(minimal_path(i,:)==1,minimal_path(i,:)==city_num) = tau(minimal_path(i,:)==city_num,minimal_path(i,:)==1);
        end
        iterate = iterate + 1;
    end
end

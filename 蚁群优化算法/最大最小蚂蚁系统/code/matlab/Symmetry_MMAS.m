...
%% --------------------------------------------------------------------%%
function [minimal_path,minimal_length] = Symmetry_MMAS(x,y,iterate_max)
    % 获取城市数目
    city_num = length(x);
    % 设置蚂蚁数目
%     ants = 20;
    ...
    while iterate < iterate_max
        ...
        for j = 2:city_num
            ...
            tau(tabu(minimal_ant,:)==j,tabu(minimal_ant,:)==j-1) = tau(tabu(minimal_ant,:)==j-1,tabu(minimal_ant,:)==j);
        end
        tau(tabu(minimal_ant,:)==city_num,tabu(minimal_ant,:)==1) = tau(tabu(minimal_ant,:)==city_num,tabu(minimal_ant,:)==1) + 1/minimal_length;
        tau(tabu(minimal_ant,:)==1,tabu(minimal_ant,:)==city_num) = tau(tabu(minimal_ant,:)==city_num,tabu(minimal_ant,:)==1);
        % 计算信息素上限

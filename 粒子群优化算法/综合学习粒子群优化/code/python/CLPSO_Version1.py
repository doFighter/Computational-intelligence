...
def CLPSO_Version1(N, dim, x_min, x_max, iterate_max, fitness):
    """
    CLPSO版本一：广泛学习粒子群优化
    :param N: 粒子种群大小
    :param dim: 问题解的维度
    ...
    while iterate < iterate_max:
        ...
        for i in range(N):
            if flag[i] >= m:
                v[i, :] = omega * v[i, :] + c[0] * np.random.random([1, dim]) * (
                    pBest[i, :] - x[i, :]) + c[1] * np.random.random([1, dim]) * (gBest - x[i, :])
                flag[1] = 0
            pBest_fi = pBest[i, :]
...
# 函数测试
for i in range(10):
    result = CLPSO_Version1(20, 30, -10, 10, 1000, Sphere)
    print(result)

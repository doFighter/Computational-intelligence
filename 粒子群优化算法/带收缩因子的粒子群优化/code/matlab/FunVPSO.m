%% =====================================================================%%
%% 带收缩因子的粒子群优化(DOI:10.1109/CEC.2002.1004493)
% coding：陈小斌
...
% fitness:评价函数
%% --------------------------------------------------------------------%%
function[rest]=FunVPSO(N,dim,x_max,x_min,iterate_max,fitness)
    chi = 0.729844;
    varphi = 2.05;
    ...
    while iterate < iterate_max+1
        ...
        v = chi*(v + varphi*rand(N,dim).*(pBest-x) + varphi*rand(N,dim).*(gBest-x));
        v(v>v_max) = v_max;
        v(v<v_min) = v_min;
        x = x + v;
        x(x>x_max) = x_max;
